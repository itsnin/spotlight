...
const SEARCH_ENGINES: {id: string; label: string}[] = [
...
export default class SpotlightPreferences extends ExtensionPreferences {
    fillPreferencesWindow(window: any): void {
        ...
        eventController.connect('key-pressed', (_controller: any, keyval: number, _keycode: number, state: number) => {
            if (!capturing)
                return false;
    ...
    _formatShortcut(shortcutArray: string[]): string {
