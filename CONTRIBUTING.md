...
the codebase is 22 javascript files 18 at the root level for the shell process and 4 inside `prefs/` for the preferences process extension.js must live at the root of the zip for the gnome extensions website to find it the prefs files are isolated under `prefs/` because they run in a separate process and must not import shell-only libraries like st clutter meta or shell
...
each preference page is in its own file, under `prefs/` since these only run in the
preferences process and must stay isolated from shell-only code (`St`, `Clutter`, etc.)
