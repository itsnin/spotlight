...
class SpotlightPopup extends St.Widget {
    _init(settings) {
        ...
        // Backup for ControlsManager._onSearchChanged override.
        this._bkpOnSearchChanged = null;
        this._onSearchChangedBound = null;
    ...
    stealOverviewSearch() {
        ...
        // The ControlsManager reacts to searchActive by fading out the app display
        // and workspaces display while fading in the search controller. Since we permanently stole the search controller widgets, this would render as a black screen when our popup is open. Override _onSearchChanged on the controls manager instance to skip those UI transitions. Source: GNOME/gnome-shell js/ui/overviewControls.js
        const controls = Main.overview._overview._controls;
        this._bkpOnSearchChanged = controls._onSearchChanged;
        this._onSearchChangedBound = this._bkpOnSearchChanged.bind(controls);
        controls._onSearchChanged = () => {
            if (this._visible)
                return;
            this._onSearchChangedBound();
        };
        ...
        // permanently stole the search widgets this would show a blank screen. We intercept printable keys at stage level. When our popup is closed we consume them. When open, we forward to our entry manually because EVENT_STOP at capture phase prevents normal target-phase delivery. The ControlsManager UI reaction to searchActive is blocked separately via the _onSearchChanged override.
    ...
    returnOverviewSearch() {
        ...
        // Restore ControlsManager._onSearchChanged.
        if (this._bkpOnSearchChanged !== null) {
            Main.overview._overview._controls._onSearchChanged = this._bkpOnSearchChanged;
            this._bkpOnSearchChanged = null;
            this._onSearchChangedBound = null;
        }
