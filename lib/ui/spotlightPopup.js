...
class SpotlightPopup extends St.Widget {
    _init(settings) {
        ...
        // Backup fields for thumbnail scale modifications.
        this._bkpMaxThumbnailScale = null;
        this._bkpSecondaryGetThumbnailsHeight = null;

        // Backup ease methods on ControlsManager actors.
        this._bkpAppDisplayEase = null;
        this._bkpWorkspacesEase = null;
        this._bkpSearchControllerEase = null;

        // Backup fields for WorkspaceThumbnail prototype overrides.
    ...
    stealOverviewSearch() {
        ...
        // The ControlsManager reacts to searchActive by calling _onSearchChanged()
        // which eases the app display and workspaces display to opacity 0 and the search controller to opacity 255. Since we permanently stole the search controller widgets, fading it in would render as a black screen. We override ease() on the three actors individually to block only those fade animations while our popup is visible. The rest of _onSearchChanged runs normally, which is needed for results to render properly. Source: GNOME/gnome-shell js/ui/overviewControls.js
        const controls = Main.overview._overview._controls;

        this._bkpAppDisplayEase = controls._appDisplay.ease;
        controls._appDisplay.ease = (...args) => {
            if (this._visible)
                return;
            return this._bkpAppDisplayEase.apply(controls._appDisplay, args);
        };

        this._bkpWorkspacesEase = controls._workspacesDisplay.ease;
        controls._workspacesDisplay.ease = (...args) => {
            if (this._visible)
                return;
            return this._bkpWorkspacesEase.apply(controls._workspacesDisplay, args);
        };

        this._bkpSearchControllerEase = controls._searchController.ease;
        controls._searchController.ease = (...args) => {
            if (this._visible)
                return;
            return this._bkpSearchControllerEase.apply(controls._searchController, args);
        };
        ...
        // permanently stole the search widgets this would show a blank screen. We intercept printable keys at stage level. When our popup is closed we consume them. When open, we forward to our entry manually because EVENT_STOP at capture phase prevents normal target-phase delivery. The ControlsManager fade animations that cause black screen are blocked separately via ease() overrides on the affected actors.
        ...
        // Increase workspace thumbnail scale to 200%. Doubles the default maximum
        // scale so thumbnails are actually usable on modern high-resolution displays.
        ...
        // Also apply to secondary monitors. The prototype method needs to know
        // our scale factor when it calculates available thumbnail height.
        ...
        // Restore workspace thumbnails background. Override WorkspaceThumbnail
        // prototype to create a BackgroundManager for each thumbnail contents container, replacing the solid grey default with the actual wallpaper. Queue relayout on load to work around a Shell 50 bug where thumbnails stay blank until something else forces a relayout.
    ...
    returnOverviewSearch() {
        ...
        // Restore original thumbnail scale values.
        ...
        // Restore ease methods on ControlsManager actors.
        const controls = Main.overview._overview._controls;
        if (this._bkpAppDisplayEase !== null) {
            controls._appDisplay.ease = this._bkpAppDisplayEase;
            this._bkpAppDisplayEase = null;
        }
        if (this._bkpWorkspacesEase !== null) {
            controls._workspacesDisplay.ease = this._bkpWorkspacesEase;
            this._bkpWorkspacesEase = null;
        }
        if (this._bkpSearchControllerEase !== null) {
            controls._searchController.ease = this._bkpSearchControllerEase;
            this._bkpSearchControllerEase = null;
        }

        // Restore WorkspaceThumbnail prototypes.
