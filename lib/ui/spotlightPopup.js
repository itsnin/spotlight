...
class SpotlightPopup extends St.Widget {
    _init(settings) {
        ...
        // Backup fields for parent container visibility.
        this._entryParentVisible = null;
        this._searchParentVisible = null;

        // Backup fields for WorkspaceThumbnail prototype overrides.
        this._bkpThumbInit = null;
        this._bkpThumbOnDestroy = null;

        // Signal ID for search-active reactive defense.
        this._searchActiveNotifyId = 0;
    ...
    stealOverviewSearch() {
        ...
        // permanently stole the search widgets this would show a blank screen. We intercept printable keys at stage level. When our popup is closed we consume them. When open, we forward to our entry manually. The overview handler fires first in the capture chain regardless of what we return, so we also hide parent containers and react to search-active changes as a second line of defense.
        ...
        // Reactive defense against black screen. The overview type-to-search handler
        // fires first due to earlier connection and sets searchActive on the controller. We react immediately and re-hide the containers if the overview tried to show them while our popup is open, working around the ControlsManager potentially switching to a search view layout.
        this._searchActiveNotifyId = this._search.connect('notify::search-active', () => {
            if (this._visible && this._search.searchActive) {
                if (this._entryParent)
                    this._entryParent.visible = false;
                if (this._searchParent)
                    this._searchParent.visible = false;
            }
        });
    ...
    returnOverviewSearch() {
        if (this._entry) {
            ...
            if (this._entryParentVisible !== null) {
                this._entryParent.visible = this._entryParentVisible;
                this._entryParentVisible = null;
            }
            this._entry = null;
            this._entryParent = null;
        ...
        if (this._search) {
            ...
            if (this._searchParentVisible !== null) {
                this._searchParent.visible = this._searchParentVisible;
                this._searchParentVisible = null;
            }
            this._search = null;
            this._searchParent = null;
        ...
        // Disconnect the reactive search-active defense.
        if (this._searchActiveNotifyId !== 0) {
            this._search.disconnect(this._searchActiveNotifyId);
            this._searchActiveNotifyId = 0;
        }
