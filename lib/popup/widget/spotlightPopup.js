...
class SpotlightPopup extends St.Widget {
    ...
    _doClose() {
        ...
        this._visible = false;
        // Removing the children first collapses the container to its minimum
        // height, so the fade animated an empty box for a frame.
        this._animateClose(() => {
            // A reopen during the fade reparents the entry, so leave the
            // contents alone if the popup is visible again.
            if (this._visible)
                return;
    ...
    _animateClose(onComplete) {
        ...
        this.ease({
            ...
            onComplete: () => {
                this.hide();
                // Reset properties so the next open starts from a clean state.
                this.opacity = 255;
                this.scale_x = 1.0;
                this.scale_y = 1.0;
                onComplete();
