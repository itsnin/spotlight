// Spotlight: positions the popup and shows it at the right size
...
// Sizing and positioning runs once per open, based on the empty-state height of just
...
export class PopupPositioner {
    ...
    // Queue the popup to be sized, positioned and shown. The onShown callback fires
    ...
    _center(popupWidth, monitor) {
        ...
        // Place the popup 25 percent from the top of the monitor. Clamp so the
        // top edge never goes off-screen, which matters when results expand the
        // popup to its maximum height.
        const centerY = monitor.y + monitor.height * 0.25;
        const topY = Math.floor(centerY - naturalHeight / 2);
        const minTopY = monitor.y + 20;
        this._popup.set_position(
            Math.floor(monitor.x + (monitor.width - popupWidth) / 2),
            Math.max(topY, minTopY),
        );
    }
    ...
    _animateOpen(onShown) {
        ...
        // Cancel any in-flight close animation before starting a new one.
        ...
        this._popup.scale_x = 0.96;
        this._popup.scale_y = 0.96;
        this._popup.show();
        // Matches GNOME's EASE_OUT_QUAD used in overviewControls.js. Slightly
        // shorter duration (180ms vs GNOME's 250ms) keeps the instant feel.
        this._popup.ease({
            opacity: 255,
            scale_x: 1.0,
            scale_y: 1.0,
